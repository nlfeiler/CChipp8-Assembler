...
void langCompile(std::vector<char> &ROMBytes, std::string outputFile, int returnCode) {
	...
	for (int i = 0; i < ROMBytes.size(); i++) {
		if(ROMBytes[i] == '#') inComment = !inComment; //sets the current comment status 
		//TODO: update documentation in docs to reflect the fact that comments no longer require the line to end with a pound sign

		//eat any spaces and then flush the current working token
		if(ROMBytes[i] == ' '){
			//flush the token
			if(!token.empty()){
			...
			//continue to our next loop iteration
			continue;
		}

		//if we have a new line we need to eat it and flush the token
		if(ROMBytes[i] == '\n' || ROMBytes[i] == '\r' || ROMBytes[i] == '\0'){
			inComment = false; //make sure we set inComment false in case if we are currently within a comment

			if(!token.empty()){ //flush the token
				tokens.push_back(token);
				token = "";
			}
		...
		//all new line chars and spaces should have been ate already so I might remove them from the code down here later but for now this has been left in to stop any potential unforeseen bugs
		//TODO: remove spaces and new line chars from if statement below and do proper QA testing
